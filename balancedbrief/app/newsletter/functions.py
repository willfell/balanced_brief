...
def obtain_posts():
    ...
    cur.execute(
        f"SELECT * FROM successful_posts WHERE time between %s AND %s;",
...
def determine_category_order():
    query = (
        "SELECT parent_category, category_order FROM public.parent_category_hierarchy;"
    )
    cur.execute(query)
...
def sucessful_email(user, current_date_time):
    ...
    data_tuple = (user["user_id"], True, current_date_time)
    ...
    try:
        ...
        print(
            f"Failed to insert data for user {user['user_email']} on email SUCCESS. Error: {e}"
        )
        return False
...
def unsucessful_email(user, current_date_time, failure_reason):
    ...
    data_tuple = (user["user_id"], False, failure_reason, current_date_time)
    ...
    try:
        ...
        print(
            f"Failed to insert data for user {user['user_email']} on email FAILURE. Error: {e}"
        )
        return False
...
def send_duplicate_check(user):
    ...
    if not has_been_sent:
        print(
            f"Email has not been sent to user {user['user_email']}, going through with email template creation"
        )
        return False
    else:
        print(
            f"Email has already been sent to user {user['user_email']}, moving to next user"
        )
        return True
